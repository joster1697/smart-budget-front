...
interface SaveBudgetBannerProps {
  isVisible: boolean;
  isDraft: boolean;
  monthYearStr: string;
      ...
      {isDraft ? (
        <div className="flex items-center justify-between md:justify-start gap-4 md:gap-3 w-full md:w-auto">
          ...
          {isPastMonth ? (
            ...
            <Button variant="primary" size="sm" onClick={onActivateBudget} loading={loading}>
              Activar {capitalizedMonth}
            </Button>
          )}
        </div>
      ) : (
        <Button variant="primary" size="sm" onClick={onSaveBudget} loading={loading}>
